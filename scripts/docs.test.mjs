...
// "Published pages".
//
// The dashboard is generated, so there is no committed page for Vale to
// read. The copy is checked where it is written instead: the renderer's
// own source, with its program comments blanked so that a note to a
// maintainer is not mistaken for something a reader sees.
const PAGES = ['site/render.mjs']
// Element whose TEXT is content rather than page prose.
const PAGE_CONTENT = /<(script|style|pre|code|kbd|textarea)\b[^>]*>[\s\S]*?<\/\1>/gi
// A renderer's source carries program comments as well as page copy.
const PAGE_COMMENT = /\/\*[\s\S]*?\*\/|^[ \t]*\/\/.*$/gm
