...
with tabnas's terminology, this repository's file layout (one README,
...
it has no per-runtime Diátaxis set; the collector and the renderer are
programs, and the report they read and write is data, not prose. The page
a reader actually sees is generated, and is covered separately under
...
The dashboard's own copy is not exempt: it is read by somebody deciding
whether to trust a published score, and a citation they cannot follow
tells them nothing.
...
matter is a gate people learn to switch off. Each was counted over the
gated set before it was left out:

Count a candidate over this repository's gated set before adding
it, and record that number beside it. `surface`, `harness`, `guarantee`
...
describes. The dashboard's own copy lives in `site/render.mjs`, which writes
`_site/`, so the page a reader sees is generated rather than
committed.
...
reading the renderer's own source so that the copy is checked
where it is written rather than after a build.

A cell that reports no value renders as a bare dash, and a date
range joins its two ends with one. Neither is a dash in prose,
